...
from qb_wbc.qp.hoqp import HoQP
from qb_wbc.qp.wqp import WQP
from qb_wbc.qp.task import Task
...
class TorqueWBC:
    def __init__(self, model: PModel, data: PData):
        ...
        self.reduced_friction_pyramid = np.array([1, 0, 0])
    ...
    def _formulate_tasks(self):
        ...
        self.hoqp.add_tasks(0, [
            self._formulate_floating_base_dynamics_constraint,
            self._formulate_contact_no_slip_constraint,
            # self._formulate_friction_cone_constraint,
            self._formulate_torque_limit_constraint,
            self._formulate_digit_mechanism_limit_task
        ]),
        self.hoqp.add_tasks(1, [
            self._formulate_lock_base_accel_task,
        ])
        self.hoqp.add_tasks(2, [
            self._formulate_optimal_control_task,
            # self._formulate_optimal_contact_task
        ])

        self.wqp.add_weighted_task(self._formulate_lock_base_accel_task, 1.0)
        # self.wqp.add_weighted_task(self._formulate_optimal_control_task, 0.3)

        self.wqp.add_constraint(self._formulate_floating_base_dynamics_constraint)
        self.wqp.add_constraint(self._formulate_torque_limit_constraint)
        # self.wqp.add_constraint(self._formulate_friction_cone_constraint)
        # self.wqp.add_constraint(self._formulate_contact_no_slip_constraint)
    ...
    def solve(self) -> Optional[WBCSolution]:
        x = self.hoqp.solve()
        return WBCSolution(
            qdd=x[0:self.nv],
            contact_forces=[x[self.nv + i * 3:self.nv + (i + 1) * 3] for i in range(self.ncont)],
            torque=x[self.nv + 3 * self.ncont:]) if x is not None else None
    ...
    def _update_system_status(self) -> None:
        ...
        self.wqp.update_n_decision(self.ndes)

    def _formulate_floating_base_dynamics_constraint(self) -> Task:
    ...
    def _formulate_contact_no_slip_constraint(self) -> Task:
    ...
    def _formulate_lock_base_accel_task(self) -> Task:
        ...
        padding = np.zeros((6, self.ndes - 6))
        task.A = np.hstack([np.eye(6), padding])
        task.b = np.zeros(6)

        return task

    def _formulate_friction_cone_constraint(self) -> Task:
        task = Task.EMPTY(self.ndes)

        fp_size = self.reduced_friction_pyramid.shape[0]
        task.D = np.zeros((fp_size * self.ncont, self.ndes))
        for i in range(self.ncont):
            task.D[i * fp_size: (i + 1) * fp_size,
            self.nv + i * 3: self.nv + (i + 1) * 3] = self.reduced_friction_pyramid.copy()
        task.f = np.zeros(fp_size * self.ncont)
    ...
    def _formulate_torque_limit_constraint(self) -> Task:
    ...
    def _formulate_digit_mechanism_limit_task(self) -> Task:
        # ensure the achilles rod joints of digit is properly constrained.
        # q_acc, tau for joint 9,10;joint 16,17 need to be equal and in the same direction (by axis definitions)
        task = Task.EMPTY(self.ndes)

        task.A = np.zeros((2, self.ndes))
        task.A[0][9] = 1
        task.A[0][10] = -1
        task.A[1][16] = 1
        task.A[1][17] = -1
    ...
    def _formulate_optimal_contact_task(self) -> Task:
        task = Task.EMPTY(self.ndes)
        task.A = np.hstack([
            np.zeros((self.ncont * 3, self.nv)),
            np.eye(self.ncont * 3),
            np.zeros((self.ncont * 3, self.nact))
        ])
        task.b = np.zeros(self.ncont * 3)

        return task

    def _formulate_lock_contact_force_constraint(self) -> Task:
        task = Task.EMPTY(self.ndes)

        task.A = np.zeros((6, self.ndes))
        task.A[0:3 * self.ncont, self.nv:self.nv + 3 * self.ncont] = np.eye(6)
        task.b = np.array([0, 0, 48.351457999999994 * 9.8061 * 0.5, 0, 0, 48.351457999999994 * 9.8061 * 0.5])
